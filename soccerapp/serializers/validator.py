...
class CustomValidator: 
    ...
    def validate_create(self, create_data) -> Tuple:
        """ 
        Validate the data to be created (for create() method), and return the list of bets 
        to be added and total bet amount
        """

        # total amount the user bet from the list, list of bets to be saved to the database
        total_bet_amount = 0 
        saved_bet_list = []  
        ...
        for i, item_data in enumerate(create_data): 
            ...
            saved_bet_list.append(self.user_bet_class(
                **item_data, created_date=date.today(), bet_info=bet_info_obj
            ))
