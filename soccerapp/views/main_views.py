...
class Standings(APIView): 
    ...
    def get(self, request) -> Response: 
        ...
        standings = TeamRanking.objects.filter(league=LEAGUES_MAP[league_name])
    ...
    def get(self, request, match_id: int, format=None) -> Response: 
        ...
        for time_type in list(response_data.keys()): 
            ...
            info_list = MoneylineBetInfo.objects.filter(
                match=match, bet_object=bet_object, time_type=TYPE_MAP[time_type])
            info_list_serializer = MoneylineBetInfoSerializer(info_list, many=True)
            response_data[time_type] = info_list_serializer.data
...
class HandicapInfoList(APIView):
    ...
    def group_handicap_info(
        self, info_list: QuerySet[HandicapBetInfo], home_team: str
        ...
        for info in info_list: 
            handicap_cover = float(info["handicap_cover"]) # The handicap cover of this info
            if info["bet_team"] != home_team: 
                handicap_cover = -handicap_cover

            # If this handicap cover isn't already in the dictionary, add it
            if handicap_cover not in cover_to_info_dict: 
                cover_to_info_dict[handicap_cover] = { 
                    "home": None, 
                    "away": None 
                }
            # The put info ino the right place
            team = "home" if info["bet_team"] == home_team else "away"
            cover_to_info_dict[handicap_cover][team] = info
...
class TotalObjectsInfoList(APIView):
    ...
    def group_total_objects_info(
        ...
        Returns the form ```[[under num_goals1, over num_goals1], [under num_goals2, over num_goals2], ...]```
        """
        # Dictionary mapping the number of objects to list of total bets info
        total_to_info_dict = {} 
        
        for info in info_list:
            target_num_objects = info["target_num_objects"]
            if target_num_objects not in total_to_info_dict: 
                # If this number of goals isn't already in dictionary, add it
                total_to_info_dict[target_num_objects] = {
                    "under": None, 
                    "over": None
                }
            # Place the total-goals bet info into the right place of the list 
            uoo = "under" if info["under_or_over"] == "Under" else "over"
            total_to_info_dict[target_num_objects][uoo] = info
    ...
    def get(self, request, match_id: int, format=None) -> Response: 
        ...
        for time_type in list(response_data.keys()): 
            ...
            response_data[time_type] = self.group_total_objects_info(
