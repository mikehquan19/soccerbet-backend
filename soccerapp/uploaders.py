    ...
    for league_name in list(leagues.keys()): 
        ...
        for rank in api_ranks_data: 
            league_standings.append(TeamRanking(
                ...
                points=rank["points"], num_watches=rank["num_matches"], num_wins=rank["num_wins"], 
...
def upload_match_bets(arg_matches: QuerySet[Match]) -> None: 
    ...
    for match in arg_matches:  
        ...
        # one database query
        created_moneyline = MoneylineBetInfo.objects.bulk_create(moneyline_info_list) 
        ...
        for bet_info in handicap_info_data: 
            handicap_info_list.append(HandicapBetInfo(
                match=match, time_type=bet_info["time_type"], bet_object=bet_info["bet_object"],
                bet_team=bet_info["bet_team"], handicap_cover=bet_info["handicap_cover"], 
                odd=bet_info["odd"]
            ))
        ...
        for bet_info in total_info_data: 
            total_info_list.append(TotalObjectsBetInfo(
                match=match, time_type=bet_info["time_type"], bet_object=bet_info["bet_object"], 
                under_or_over=bet_info["under_or_over"], target_num_objects=bet_info["num_objects"], 
                odd=bet_info["odd"]
            ))
